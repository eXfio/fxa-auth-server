  ...
  return {
    ...
    notifyUpdate: function notifyUpdate(uid) {
      return db.devices(uid).then(
        function (devices) {
          log.trace({op: LOG_OP_NOTIFY_UPDATE, devices: devices})

          devices.forEach(function (device) {
            if (device.pushCallback) {
              log.trace({op: LOG_OP_NOTIFY_UPDATE, pushCallback: device.pushCallback})

              // send the push notification
              log.increment(LOG_UPDATE_INCREMENT_SEND)
              request.post(device.pushCallback, function (err){
                if (err) {
                  log.error({op: LOG_OP_NOTIFY_UPDATE, err: err})
              ...
              log.error({op: LOG_OP_NOTIFY_UPDATE, err: ERR_NO_PUSH_CALLBACK})
